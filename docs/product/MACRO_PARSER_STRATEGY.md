...
- `FED_FUNDS_RATE`, `DXY`, `BRENT_OIL_PRICE` (FRED API): Examined in Phase 148O/P. API requires authentication key (`auth_required`). -> `source_assessment_needed` -> **Blocked** from automated parser dry-run. The label for DXY was changed to "Sức mạnh USD" to mitigate semantic risk with the DTWEXBGS proxy. Parsers will not execute until API keys are configured and integrated.
