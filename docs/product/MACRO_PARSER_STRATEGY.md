...
- `FED_FUNDS_RATE`, `DXY`, `BRENT_OIL_PRICE` (FRED API): Examined in Phase 148O. API requires authentication key (`auth_required`). -> `source_assessment_needed` -> **Blocked** from automated parser dry-run. `DXY` candidate (`DTWEXBGS`) additionally noted as a semantic proxy requiring manual review.
