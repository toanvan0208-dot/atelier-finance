...
- Financials now has a DB-backed runtime loader boundary that prefers usable local/imported DB rows and falls back safely to sample/static data when DB rows are absent or insufficient.
...
Phase 94 hardens the Financials DB-backed UI read path. `loadFinancialsRuntimeData` now tries local/imported DB-backed financial statement rows first, returns `runtimeStatus:db_backed`/`readPath:local_db` only when usable rows are adapted, preserves missing numeric fields as null/unavailable, and falls back to static sample data when rows are absent or insufficient. Overview only treats Financials as DB-backed when the Financials runtime actually used local DB rows, and Valuation remains bounded with `canClaimValuationDbBacked:false`. No public upload UI/API, filesystem CSV read, DB write, schema migration, external API/Vnstock call, source approval, recommendation, target, fair-value, risk-scoring, official/realtime, or production-approved claim is added.

Phase 95 lets Valuation consume verified Financials runtime inputs safely. The controlled Valuation boundary accepts Financials values only when runtime metadata proves a non-fallback local DB-backed read path with source, period/as-of, and explicit units; otherwise inputs remain unavailable or fall back to existing persisted bridges. Market price and Market/PVT ownership remain separate, missing/invalid shares block share-based metrics, non-positive EPS/equity remain not-applicable, and `canClaimValuationDbBacked:false` stays enforced. Overview marks Valuation partial, not complete, when only Financials input is DB-backed. No new valuation method, public upload UI/API, filesystem CSV read, DB write, schema migration, external API/Vnstock call, source approval, recommendation, target, fair-value, risk-scoring, official/realtime, or production-approved claim is added.
