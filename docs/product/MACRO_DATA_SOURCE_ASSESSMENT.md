...
## Phase 149B Vietnam Macro Source Acquisition

Phase 149B assessed the Vietnam macro acquisition batch for `USD_VND`, `EXPORT_GROWTH`, `CREDIT_GROWTH`, and `PUBLIC_INVESTMENT`. All four are visible in the Macro frontend and remain not DB-backed. `USD_VND` has a clear alternate URL candidate (`Vietcombank Exchange Rate XML API`) and was verified reachable with HTTP 200 and `text/xml; charset=utf-8`; no numeric values were extracted and no rows were created. `EXPORT_GROWTH` has only a GSO candidate without a concrete URL, so it is blocked with `missing_source_url`. `CREDIT_GROWTH` is blocked until an official SBV/source URL and exact credit definition are selected; M2 growth or lending rates must not be substituted. `PUBLIC_INVESTMENT` is blocked until the selected source confirms whether the metric is realized public investment capital from the state budget or another approved definition. No production approval was added.
