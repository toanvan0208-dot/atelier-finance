...
- **Phase 148O Global Macro Source**: Audited `FED_FUNDS_RATE`, `DXY`, and `BRENT_OIL_PRICE`. All rely on FRED API which requires an authentication key (auth_required) and are therefore blocked from automated parser dry-run. DTWEXBGS was noted as a proxy for ICE DXY carrying semantic risk. Updated Assistant guardrail to prevent hallucinating global macro signals.
- **Phase 148P API Key & Semantic Boundary**: Finalized the API key boundary for FRED (no fetch without key). Resolved the DXY semantic risk by renaming the UI label to "Sức mạnh USD" to reflect the broad dollar index (DTWEXBGS) rather than the official ICE DXY. Hardened the unavailable states for `FED_FUNDS_RATE`, `DXY`, and `BRENT_OIL_PRICE` with explicit UI copy and Assistant guardrails: "Hiện hệ thống chưa có dữ liệu đã kiểm duyệt cho lãi suất Fed, chỉ số USD hoặc giá dầu Brent, nên không kết luận tác động đến ngành hoặc cổ phiếu từ các chỉ số này."
