...
async function analyzeCollisions(rows: NormalizedImportRow[]) {
  requirePostgresDatabaseUrl("dry-run-mwg-pdf-reviewed-import.ts");
...
async function run() {
  requirePostgresDatabaseUrl("dry-run-mwg-pdf-reviewed-import.ts");
  console.log("Phase 139H — MWG PDF reviewed preview-to-import dry-run only");
  console.log("=================================================================\n");

  const jsonPath = path.join(process.cwd(), "docs/product/evidence/PHASE139H_MWG_PDF_2025_PREVIEW.json");
  if (!fs.existsSync(jsonPath)) {
    console.error("Phase 139H preview JSON not found!");
    process.exit(1);
  }
