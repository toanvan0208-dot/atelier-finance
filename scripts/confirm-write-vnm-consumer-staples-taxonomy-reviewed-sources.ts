...
const sharedValidationReasons = (row: {
  ...
  for (const code of REQUIRED_WARNING_CODES) {
...
async function main() {
  ...
  if (scopedPeerGroupPackages.length > 0) {
    structuralBlocks.push({
      ...
      reasons: ["INDUSTRY_PEER_GROUP_WRITE_NOT_ALLOWED_IN_PHASE_150T"],
    });
  }
  ...
  const [
    ...
    vnmPeerGroupRowsBefore,
    hpgSteelPeerGroupRowsBefore,
    mwgRetailRowsBefore,
  ...
  const [
    ...
    vnmPeerGroupRowsAfter,
    hpgSteelPeerGroupRowsAfter,
    mwgRetailRowsAfter,
  ...
  const idempotencyPassed =
    ...
    vnmPeerGroupRowsAfter === vnmPeerGroupRowsBefore &&
  ...
  const result = {
    ...
    guardrailResults: {
      onlyIndustryAndCompanyIndustryWrites:
        !confirmWriteRequested ||
        industryRowsCreated + industryRowsUpdated + companyIndustryRowsCreated + companyIndustryRowsUpdated > 0,
      noPeerGroupWrites: vnmPeerGroupRowsAfter === vnmPeerGroupRowsBefore && scopedPeerGroupPackages.length === 0,
      noIndustryContextWrites: industryContextRowsAfter >= 0 && industryContextProvenanceRowsAfter >= 0,
      noIndustryMetric: !industryMetricCreated,
      hpgPeerGroupIntact: hpgSteelPeerGroupRowsAfter === hpgSteelPeerGroupRowsBefore && hpgSteelPeerGroupRowsAfter === 3,
      mwgRetailTaxonomyIntact: mwgRetailRowsAfter === mwgRetailRowsBefore && mwgRetailRowsAfter >= 1,
  ...
  const smokePassed =
    ...
    result.vnmPeerGroupRowsAfter === 0 &&
    result.hpgSteelPeerGroupRowsAfter === 3 &&
    result.mwgRetailRowsAfter >= 1 &&
