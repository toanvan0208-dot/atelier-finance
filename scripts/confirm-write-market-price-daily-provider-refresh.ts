...
async function runDailyProviderRefreshConfirmWrite() {
    const isConfirmWrite = process.argv.includes("--confirm-write");
    const mode = "market_price_daily_refresh_confirm_write";
    ...
    let recommendedNextPhase = "Phase 145Z - Re-run with confirm-write";
    if (isConfirmWrite) {
...
runDailyProviderRefreshConfirmWrite().catch(err => {
