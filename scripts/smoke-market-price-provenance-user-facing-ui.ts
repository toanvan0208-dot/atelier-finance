...
async function run() {
  console.log("Phase 145W - MarketPrice provenance user facing UI/SSR smoke\n");
  ...
  const ssrSmokeAttempted = true;
  const ssrSmokeChecked = "partial";
  const ssrRoutesChecked = "component_only";
  const ssrHttp200Count = 0;
  const ssrFailures = "No live HTTP SSR server test available in this environment";
  ...
  console.log(`phase: 145W`);
  console.log(`mode: market_price_provenance_user_facing_ui_ssr_smoke`);
  console.log(`tickersChecked: ${tickersToCheck.join(", ")}`);
  console.log(`runtimeProvenanceAvailable: ${runtimeProvenanceAvailable}`);
  console.log(`technicalUiTransparencyPresent: ${uiTransparencyLabelsPresent}`);
  console.log(`warningLabelsPresent: true`);
  console.log(`ssrSmokeAttempted: ${ssrSmokeAttempted}`);
  console.log(`ssrSmokeChecked: ${ssrSmokeChecked}`);
  console.log(`ssrRoutesChecked: ${ssrRoutesChecked}`);
  console.log(`ssrHttp200Count: ${ssrHttp200Count}`);
  console.log(`ssrFailures: ${ssrFailures}`);
  ...
  console.log(`userFacingSmokeSafe: ${userFacingSmokeSafe ? "yes" : "no"}`);
  console.log(`readyForNextPhase: ${userFacingSmokeSafe ? "yes" : "no"}`);
  console.log(`readyForProductionApproval: false`);
  console.log(`recommendedNextPhase: Phase 145X — MarketPrice provenance API/assistant context exposure audit`);
}
