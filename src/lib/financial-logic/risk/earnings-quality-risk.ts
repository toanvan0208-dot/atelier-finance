...
export const calculateEarningsQualityRisk = (input: FinancialStatementInput): RiskScoreResult => {
  ...
  const score = mismatch ? 70 : marginWeak || cfo.level === "risk" ? 45 : cfo.level === "watch" ? 25 : 10;
  return {
    ...
    warnings,
    missingFields: Array.from(new Set([cfo, revenueGrowth, profitGrowth, netMargin].flatMap((metric) => metric.missingFields))),
