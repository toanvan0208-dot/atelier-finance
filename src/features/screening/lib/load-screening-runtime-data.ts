...
const NO_ZERO_FILL_WARNING = "Chưa đủ dữ liệu để kết luận. Hệ thống không tự điền 0 hoặc suy đoán dữ liệu thiếu.";
const SOURCE_WARNING = "Dữ liệu hiện tại là dữ liệu nghiên cứu hoặc đã rà soát thủ công, chưa phải dữ liệu chính thức để ra quyết định.";
...
function buildExcludedCandidate(ticker: string): RedesignedScreeningCandidate {
  return {
    ...
    explanationForBeginner: "Cổ phiếu ngân hàng có mô hình kinh doanh và chuẩn mực kế toán riêng biệt, không thể dùng bảng lọc hiện tại để đánh giá.",
    whatToCheckNext: [],
...
export const loadScreeningRuntimeData = async (
  ...
  for (const ticker of approvedTickers) {
    ...
    if (financialsData) {
      ...
      const valuationBoundary = buildControlledValuationIntegrationBoundary({
        financialsRuntimeSnapshot: {
          ...
          fiscalYear: 2025, // Mock default to pass schema
          period: record?.period ?? "2025",
          periodType: record?.periodType ?? "year",
          productionApproved: false,
          readPath: financialsData.source.readPath,
          revenue: record?.revenue,
          runtimeStatus: financialsData.source.fallbackUsed ? "not_connected" : "db_backed",
    ...
    if (hasCompanyInfo) availableFields.push("Thông tin doanh nghiệp"); else missingFields.push("Thông tin doanh nghiệp");
    if (businessProfile) availableFields.push("Ngành liên quan"); else missingFields.push("Ngành liên quan");
    if (hasFinancials) availableFields.push("Dữ liệu tài chính nghiên cứu"); else missingFields.push("Dữ liệu tài chính");
    if (hasEps) availableFields.push("EPS candidate"); else missingFields.push("EPS");
    if (hasDebt) availableFields.push("Nợ vay candidate"); else missingFields.push("Nợ vay");
    if (hasShares) availableFields.push("Số cổ phiếu candidate"); else missingFields.push("Số lượng cổ phiếu");
    ...
    const group = (hasCompanyInfo && hasFinancials && hasEps && hasDebt && hasShares) ? "priority" 
                : (hasCompanyInfo || hasFinancials) ? "watch" : "not-fit";
    ...
    candidates.push({
      ...
      warnings: [SOURCE_WARNING, NO_ZERO_FILL_WARNING],
      explanationForBeginner: isMissing ? "Chưa đủ dữ liệu để đánh giá cổ phiếu này." : "Mã này có đủ dữ liệu tối thiểu để tiếp tục đọc doanh nghiệp, BCTC, định giá và rủi ro; trạng thái này không nói cổ phiếu tốt/xấu.",
      ...
      group,
      groupLabel: group === "priority" ? "Đủ dữ liệu để phân tích tiếp" : group === "watch" ? "Cần bổ sung dữ liệu" : "Chưa đủ dữ liệu",
      fitLabel: group === "priority" ? "Dữ liệu có thể phân tích tiếp" : "Thiếu dữ liệu quan trọng",
      reason: group === "priority" ? "Có đủ nhóm dữ liệu tối thiểu để chuyển sang bước sau." : "Hệ thống đang thiếu dữ liệu nền tảng cho mã này.",
      checkFlags: missingFields,
      nextStep: group === "priority" ? "Phân tích tiếp ở module doanh nghiệp hoặc xem dữ liệu tài chính." : "Chờ dữ liệu hoặc kiểm tra thủ công.",
      metrics: {
