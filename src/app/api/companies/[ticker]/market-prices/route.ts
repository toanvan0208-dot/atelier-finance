...
const resolveTicker = async (context: MarketPricesRouteContext): Promise<string> => {
...
export const GET = async (
  request: Request,
  context: MarketPricesRouteContext,
): Promise<Response> => {
  try {
    ...
    if (parseLatest(url)) {
      const price = await getLatestMarketPrice(ticker, { dataMode });
      if (!price) {
        return apiError("MARKET_PRICES_NOT_FOUND", "Market prices were not found.", {
          status: 404,
          reason: "No persisted market price matched the requested ticker.",
        });
      }
